...
export function CaseDescription({ data }: { data: CaseWorkspaceData }) {
  const allFlagged = (data.panel?.biomarkers ?? [])
    .filter((b) => b.status === 'abnormal' || b.status === 'borderline');
  const flaggedTiles = allFlagged.slice(0, 6);
  const extraFlagged = allFlagged.length > 6 ? allFlagged.length - 6 : 0;
    ...
    <section style={{ padding: '18px 24px', fontFamily: 'var(--font-sans)' }}>
      ...
      >
        ...
        {flaggedTiles.length > 0 && (
          <>
            <div style={{ marginTop: 12, display: 'grid', gridTemplateColumns: `repeat(${Math.min(flaggedTiles.length, 3)}, minmax(0,1fr))`, gap: 8 }}>
              {flaggedTiles.map((b) => (
            ...
            {extraFlagged > 0 && (
              <div style={{ marginTop: 6, fontSize: 11, color: 'var(--terracotta-deep, #9C3F25)', fontWeight: 600 }}>
                + {extraFlagged} more flagged
              </div>
            )}
          </>
        )}
